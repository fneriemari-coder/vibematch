...
class AdminRepository {
  ...
  Future<AdminUserSummary> updateAccountStatus(String userId, AccountStatus status) async {
    final response = await _client.dio.patch('/admin/users/$userId/account-status', data: {
      'accountStatus': status.apiValue,
    });
    return AdminUserSummary.fromJson(response.data as Map<String, dynamic>);
  }

  Future<AdminUserSummary> setIdentityVerified(String userId, bool verified) async {
    final response = await _client.dio.patch('/admin/users/$userId/identity-verified', data: {
